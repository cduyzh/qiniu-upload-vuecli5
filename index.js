...
class Upload {
    ...
    apply(compiler) {
        ...
        ;(compiler.hooks
            ? compiler.hooks.afterEmit.tapAsync.bind(compiler.hooks.afterEmit, 'UploadQiNiuPlugin ')
            : compiler.plugin.bind(compiler, 'afterEmit'))(async (compilation, callback) => {
                ...
                try {
                    ...
                    _this.newFileList = paths.filter(
                        (e) =>
                            (!this.options.excludeHtml && e.endsWith('.html')) ||
                            !_this.fileExistList.map((i) => i.path).includes(e.replace(_this.options.uploadTarget + '/', _this.options.prefixPath))
                    )
                } catch (error) {
                    log(`Error: ${chalk.red('not found file log')}`)
            ...
            _this.newFileList.forEach((item) => {
                ...
                if (_this.needUploadArray.length === _this.fileCount) {
                    ...
                    fs.writeFileSync(
                        fd,
                        JSON.stringify(
                            _this.fileExistList.concat(
                                _this.needUploadArray
                                    .map((e) => {
                                        return {
                                            path: e.replace(this.options.uploadTarget + '/', this.options.prefixPath),
                    ...
                    log(`Uploading ${chalk.red(_this.needUploadArray.length)} files...`)
    ...
    uploadFilesByArr(arr) {
        arr.forEach((path) => {
            let filePath = path,
                key = path.replace(this.options.uploadTarget + '/', this.options.prefixPath)
...
class UploadQiNiuPlugin extends Upload {
    constructor(options) {
        ...
        this.mac = new qiniu.auth.digest.Mac(this.options.accessKey, this.options.secretKey)
    }
    getToken(bucket, key) {
        let options = {
            scope: bucket + ':' + key,
    ...
    uploadFileToQiniu(token, fileName, localFilePath) {
        ...
        formUploader.putFile(token, fileName, localFilePath, putExtra, (err, respBody, respInfo) => {
            if (err) {
    ...
    refreshInCloud(needRefreshArr = []) {
        ...
        refreshQueue.forEach((item, index) => {
            const refreshUrlMap = item.map((e) => {
                return this.options.publicPath + e.replace(this.options.uploadTarget + '/', '')
            })
            cdnManager.refreshUrls(refreshUrlMap, function (err, respBody, respInfo) {
                if (err) {
                    _this.allRefreshIsSuccess = false
                    _this.failedObj.refreshArr = _this.failedObj.refreshArr.concat(item.map((e) => e.replace(_this.options.uploadTarget + '/', '')))
...
class UploadAwsPlugin extends Upload {
    constructor(options) {
        ...
        ;(AWS.config.region = this.options.region), (this.bucket = new AWS.S3({params: {Bucket: this.options.bucket}}))
    }
    ...
    refreshAwsCloud(newFileList) {
        ...
        const newfilePath2 = []
        const newfilePath = newFileList
            .map((item) => {
                const key = '/' + item.replace(this.options.uploadTarget + '/', '')
                if (!this.options.awsDistributionId2 && !this.options.excludeHtml && key.includes('.html')) {
                    return ('/' + this.options.htmlPath + key).replace('//', '/')
                } else if (this.options.awsDistributionId2 && key.includes('.html')) {
                    newfilePath2.push(('/' + this.options.htmlPath + key).replace('//', '/'))
                    return ''
                } else {
                    return key
                }
        ...
        var params = {
            ...
            InvalidationBatch: {
                ...
                Paths: {
                    /* required */ Quantity: newfilePath.length /* required */,
                    Items: newfilePath,
                },
            },
        }
        cloudfront.createInvalidation(params, function (err, data) {
            ...
            log(`${chalk.green('刷新static-cdn域资源成功')}`)
        })
        if (newfilePath2.length) {
            var params2 = {
                DistributionId: this.options.awsDistributionId2 /* required */,
                InvalidationBatch: {
                    /* required */ CallerReference: Date.now().toString() /* required */,
                    Paths: {
                        /* required */ Quantity: newfilePath2.length /* required */,
                        Items: newfilePath2,
                    },
                },
            }
            cloudfront.createInvalidation(params2, function (err, data) {
                if (err) console.log(err, err.stack)
                else console.log(data)
                log(`${chalk.green('刷新www域html成功')}`)
            })
        }
        fs.unlinkSync(`${_this.logLocalPath}`)
        _this.callback()
